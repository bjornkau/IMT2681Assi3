...
//APIURL contains url to fixer api
var APIURL = string("http://api.fixer.io/latest")

//DISCLAIMER contains string for when certian conditions are med
...
//ResponsePayload contains the payload sendt from our service
...
//ParseInputBody returns base and target currency from request
...
//GetResponse returns value of rate, and bool saying if either base or target is Euro
...
//ParseRate returns value of rate from db response and if either base or target is Euro
...
//CreateResp returns response string
